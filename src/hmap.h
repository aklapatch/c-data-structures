...
uint64_t wrapping_mul(uint64_t a, uint64_t b){
...
uint64_t rotr(uint64_t n, int32_t c){
    uint32_t mask = (8*sizeof(n) - 1);
    c &= mask;
    return (n << c) | (n>> ( (-c)&mask));
}

uint64_t rotl(uint64_t n, int32_t c){
    uint32_t mask = (8*sizeof(n) - 1);
    c &= mask;
    return (n >> c) | (n << ( (-c)&mask));
}

void update(uint64_t * buf, uint64_t * pad, uint64_t data_in){
    uint64_t tmp = wrapping_mul( (data_in ^ *buf), C_DS_AHASH_MULTIPLE );
    *pad = wrapping_mul(rotl((*pad ^ tmp), 8) , C_DS_AHASH_MULTIPLE);
    *buf = rotl((*buf ^ *pad), 24);
}

void update_128(uint64_t * buf, uint64_t * pad, uint64_t data_in[2]){
    update(buf, pad, data_in[0]);
    update(buf, pad, data_in[1]);
}

uint64_t ahash_buf(uint8_t *data, size_t data_len){
    uint64_t buffer = C_DS_HASH_SEED1, pad = C_DS_HASH_SEED2;

    buffer = wrapping_mul(C_DS_AHASH_MULTIPLE, wrapping_add((uint64_t)data_len, buffer));

    if (data_len > 8){
        if (data_len > 16){
            // update on the last 128 bits
            uint64_t *last_128 = (uint64_t*)(&data[data_len - 17]);
            update_128(&buffer, &pad, last_128);
            while (data_len > 16){
                update(&buffer, &pad, *(uint64_t*)data);
                data += sizeof(uint64_t);
                data_len -= sizeof(uint64_t);
            }
        } else {
            update(&buffer, &pad, *(uint64_t*)data);
            update(&buffer, &pad, *(uint64_t*)&data[data_len - 9]);
        }
    }else{
        uint64_t vals[2] = {0};
        if (data_len >= 2){
            if (data_len >= 4){
                vals[0] = *(uint32_t*)data;
                vals[1] = *(uint32_t*)&data[data_len - 5];
            } else {
                vals[0] = *(uint16_t*)data;
                vals[1] = *(uint16_t*)&data[data_len - 3];
            }
        } else {
            if (data_len > 0){
        ...
        update_128(&buffer, &pad, vals);
    }

    uint32_t rot = buffer & 63;
    return rotl(wrapping_mul(C_DS_AHASH_MULTIPLE, buffer) ^ pad, rot);
}
